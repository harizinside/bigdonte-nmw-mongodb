...
const TableThreeteen = () => {
  ...
  //     setLoading(true);
  //     try {
  //       // Panggil API pertama: Services Catalog
  //       const resCatalog = await fetch(`/api/services/servicesTwo/${id}`);
  //       if (!resCatalog.ok) {
  //         throw new Error(`Gagal mengambil data katalog: ${resCatalog.statusText}`);
  //       }
  //       const catalogData = await resCatalog.json();
        
  //       if (catalogData.data && Array.isArray(catalogData.data) && catalogData.data.length > 0) {
  //         setServices(catalogData.data);
  //         // Misalnya, ambil slug dari service pertama
  //         const slug = catalogData.data[0].slug;
  //         if (slug) {
  //           // Panggil API kedua: Service Detail dengan slug
  //           const resDetail = await fetch(`/api/services/servicesTwoDetail/${slug}`);
  //           if (!resDetail.ok) {
  //             throw new Error(`Gagal mengambil data detail: ${resDetail.statusText}`);
  //           }
  //           const detailData = await resDetail.json();
  //           if (detailData.data) {
  //             setServices(detailData.data);
  //           }
  //         }
  //       }
  //     } catch (error) {
  ...
  const fetchService = async () => {
    try {
      const response = await fetch(`/api/services/servicesTwo/${id}`);
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
      const result = await response.json();
      const servicesData = result.data; // Misalnya array service
  
      // Lakukan fetch detail untuk setiap service dan gabungkan hasilnya ke dalam objek service
      const servicesWithDetail = await Promise.all(
        servicesData.map(async (service: any) => {
          try {
            const detailRes = await fetch(`/api/services/servicesTwoDetail/${service.slug}`);
            if (!detailRes.ok) {
              throw new Error(`HTTP error! Status: ${detailRes.status}`);
            }
            const detailResult = await detailRes.json();
            // Gabungkan detail (misalnya, properti description, phone, dll) ke dalam service
            return { ...service, ...detailResult.data };
          } catch (error) {
            console.error("Error fetching detail for service id", service.id, error);
            // Jika gagal fetch detail, kembalikan service tanpa detail
            return service;
          }
        })
      );
  
      setServices(servicesWithDetail);
    } catch (error) {
  ...
  const handleDeleteServices = async (id: string | number) => {
    try {
      ...
      const response = await fetch(`/api/services/servicesTwoDelete/${id}`, {
        method: 'DELETE',
      });
      ...
      {loading ? (
        ...
        <table className="w-full table-auto">
          ...
          {services.map((service, index) => {
            const detail: ServiceDetail = serviceDetails[service.id] || {};
            return (
              <tr key={service.id}>
                {/* No. */}
                <td
                ...
                >
                  <div className="h-auto w-30">
                    <Image
                      src={`https://nmw.prahwa.net/storage/${detail.image || service.image}`} // gunakan detail jika ada, jika tidak fallback ke service.image
                      width={300}
                      height={300}
                      alt={service.title}
                      priority
                ...
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <Link href={`/services/list/${id}/edit/${service.slug}`} className="p-0 m-0 flex items-center justify-center">
                    ...
                    <Link href={`/services/list/${id}/${service.id}`} className="p-0 m-0 flex items-center justify-center">
                      <button className="hover:text-orange-400">
                        <svg
