...
const TableThree: React.FC<TableProps> = ({ limit = null, showPagination = true }) => {
    ...
    const handleDeleteProduct = async (_id: string | number) => {
          ...
          <tbody>
            {products.map((product: any, index) => (
              <tr key={index}>
                ...
                <td className={`border-[#eee] px-4 py-4 dark:border-dark-3 w-10 xl:pl-7.5 ${index === products.length - 1 ? "border-b-0" : "border-b"}`}>
                ...
                <td
                  className={`border-[#eee] px-4 py-4 dark:border-dark-3 w-95 xl:pl-0 ${index === products.length - 1 ? "border-b-0" : "border-b"}`}
                >
                ...
                >
                  <div className="flex items-center justify-end space-x-3.5">
                    <Link href={`/dashboard/products/edit/${product._id}`} className="p-0 m-0 flex items-center justify-center">
                    ...
                    <button className="hover:text-red" onClick={() => { setSelectedProduct(product); setIsOpen(true); }}>
                      <svg
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-35 z-999 flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl p-6 py-9 w-1/3 shadow-lg">
            ...
            <div className="flex justify-center gap-3">
              <button className="bg-gray-200 hover:bg-gray-300 text-lg text-gray-600 py-2 px-5 rounded-lg cursor-pointer" onClick={() => setSelectedProduct(null)}>
                Cancel
              </button>
              <button className="bg-red-500 hover:bg-red-600 text-lg text-white py-2 px-5 rounded-lg cursor-pointer" onClick={() => handleDeleteProduct(selectedProduct._id)}>
