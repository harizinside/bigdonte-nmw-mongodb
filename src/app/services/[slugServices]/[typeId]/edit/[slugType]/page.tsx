...
const EditServiceList = () => {
    const { id, typeId, slugType } = useParams();
    const router = useRouter();
    
    const [service, setService] = useState({ title: "", description: "", image: ""});
    ...
    useEffect(() => {
        ...
        const fetchService = async () => {
          setLoading(true);
          try {
            const res = await fetch(`/api/services/servicesThreeDetail/${slugType}`);
        ...
        const handleUpdate = async (e: React.FormEvent) => { 
          ...
          try {
            const res = await fetch(`/api/services/servicesThreeEdit/${id}`, {
              method: "POST",
              body: formData, 
            });
    ...
    const handlePush = () => {
      setIsOpen(false);
      router.push(`/services/list/${id}/${typeId}`);
    }
  ...
  return (
    <DefaultLayout>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Breadcrumb route="services" pageName="Manage Services" pageNameSecond="/ Manage List Services" pageNameThird="/ Type" pageNameFour={`/ Edit ${slugType}`} pageNameFive=""/>
        </div>
              ...
              <div className="p-6.5 ">
                ...
                <div className="flex gap-3 mt-7">
                    <button type="submit" disabled={loading} className="flex w-max justify-center gap-2 rounded-[7px] bg-green p-[9px] px-5 font-medium text-white hover:bg-opacity-90">
                        ...
                        {loading ? "Saving..." : "Save List Type Service"}
                    </button>
