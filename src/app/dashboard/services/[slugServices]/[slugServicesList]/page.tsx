...
const Services = () => { 
  const { slugServices, slugServicesList } = useParams();
  ...
  return (
    <DefaultLayout>
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Breadcrumb route="dashboard/services" pageName="Manage Services" routeSecond={`dashboard/services/${slugServices}`} pageNameSecond={`/ ${formattedTitle}`} routeThird={`dashboard/services/${slugServices}/${slugServicesList}`} pageNameThird={`/ ${formattedTitleList}`} routeFour="" pageNameFour="" routeFive="" pageNameFive="" />
            <ButtonDefault
                    label="Add New Type List Services"
                    link={`/dashboard/services/${slugServices}/${slugServicesList}/create`}
