...
const CreateServiceType = () => { 
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (response.status === 201) {
          setIsOpen(true);
          setMessage("Services Type berhasil ditambahkan!");
          setName("");
          setSlug("");
          setDescription("");
          setImage(null);
        } else {
          setMessage("Gagal menambahkan Services Type.");
        }
      } catch (error) {
  ...
  return (
    <DefaultLayout>
      ...
      <Breadcrumb route="dashboard/services" pageName="Manage Services" routeSecond={`dashboard/services/${slugServices}`} pageNameSecond={`/ ${formattedTitle}`} routeThird={`dashboard/services/${slugServices}/${slugServicesList}`} pageNameThird={`/ ${formattedTitleList}`} routeFour="" pageNameFour="/ Create" routeFive="" pageNameFive="" />      </div>
              ...
              <div className="p-6.5 ">
                ...
                <div className="flex gap-3 mt-1">
                    <button onClick={handleSubmit} disabled={loading} className="flex w-max justify-center gap-2 rounded-[7px] bg-green p-[9px] px-5 font-medium text-white hover:bg-opacity-90">
                        ...
                        {loading ? "Saving..." : "Save Service Type"}
                    </button>
