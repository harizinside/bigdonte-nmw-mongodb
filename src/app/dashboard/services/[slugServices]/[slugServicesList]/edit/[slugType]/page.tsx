...
const EditServiceType = () => {
    const { slugServices, slugServicesList, slugType } = useParams(); // Ambil slug service dari URL
    const router = useRouter();
  
    const [service, setService] = useState({ name: "", slug: "",description: "",  image: ""});
    ...
    useEffect(() => {
      ...
      const fetchService = async () => {
        try {
          const response = await fetch(`/api/servicesType/${slugType}`, {
            method: "GET",
            headers: {
    ...
    const handleUpdate = async (e: React.FormEvent) => { 
        ...
        try {
          ...
          setMessage("Service Type successfully updated!");
          setIsOpen(true);
        } catch (error) {
  ...
  const handlePush = () => {
    setIsOpen(false);
    router.push(`/dashboard/services/${slugServices}/${slugServicesList}`);
  }
  ...
  const formattedTitleType = formatSlugToTitle(Array.isArray(slugType) ? slugType[0] : slugType);

  return (
    <DefaultLayout>

      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Breadcrumb route="dashboard/services" pageName="Manage Services" routeSecond={`dashboard/services/${slugServices}`} pageNameSecond={`/ ${formattedTitle}`} routeThird={`dashboard/services/${slugServices}/${slugServicesList}`} pageNameThird={`/ ${formattedTitleList}`} routeFour="" pageNameFour=" / Edit" routeFive="" pageNameFive={`/ ${formattedTitleType}`} />
      </div>
            ...
            <form onSubmit={handleUpdate} encType="multipart/form-data">
              <div className="p-6.5 ">
                ...
                <div className="flex gap-3 mt-1">
                    ...
                    <Link href={`/dashboard/services/${slugServices}/${slugServicesList}`}>
