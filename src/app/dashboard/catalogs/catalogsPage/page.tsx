...
const CatalogsPage = () => {
    ...
    const [catalogsPage, setcatalogsPage] = useState<CatalogsPage | null>(null);
    ...
    const [previewImage, setPreviewImage] = useState<string | null>(null);
      ...
      useEffect(() => {
        ...
        const fetchCatalogsPage = async () => {
          try {
            const response = await fetch(`/api/catalogsPage`, {
              method: "GET",
              headers: {
      ...
      const handleUpdate = async (e: React.FormEvent) => { 
        ...
        formData.append("title", catalogsPage.title);
        formData.append("headline", catalogsPage.headline);
        formData.append("description", catalogsPage.description);
      
        if (image) {
          formData.append("image", image);
        }  

        catalogsPage.keywords.forEach(keywords => formData.append("keywords", keywords));
      
        try {
          const res = await fetch(`/api/catalogsPage`, {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_SECRET_KEY}`,
            },
  ...
  return (
    <DefaultLayout>
      <div className="mb-6 flex flex-col gap-3 sm:flex-col sm:items-start sm:justify-between">
        <Breadcrumb route="dashboard/catalogs" pageName="Manage Catalogs" routeSecond={`dashboard/catalogs/catalogsPage`} pageNameSecond="/ Catalogs Page" routeThird="" pageNameThird="" routeFour="" pageNameFour="" routeFive="" pageNameFive="" />
      </div>
                ...
                <form onSubmit={handleUpdate} encType="multipart/form-data">
                    <div className="mb-4 flex flex-col gap-4.5 xl:flex-row ">
                        ...
                        <div className="flex flex-col items-center justify-center">
                            ...
                            {(previewImage || catalogsPage?.image) && (
                            <Image
                                width={800}
                                height={800}
                                src={(previewImage || catalogsPage?.image) as string}
                                alt="Preview"
                                priority
                    ...
                    <div className="mb-7 flex flex-col gap-4.5 xl:flex-row">
                        <div className="w-full xl:w-1/2">
                            ...
                            <input
                                type="text"
                                defaultValue={catalogsPage?.title}
                                name="title"
                        ...
                        <div className="w-full xl:w-1/2">
                            ...
                            <input
                                type="text"
                                defaultValue={catalogsPage?.headline}
                                name="headline"
                                onChange={handleChange}
                    ...
                    <div className="mb-6 flex flex-col gap-4.5 xl:flex-row">
                        <div className="w-full xl:w-full">
                            ...
                            <textarea
                                defaultValue={catalogsPage?.description}
                                name="description"
                                onChange={handleChange}
                    ...
                    <div className="flex gap-3">
                        <button type="submit" onClick={handleUpdate} disabled={updating} className="flex w-max gap-2 justify-center rounded-[7px] bg-green-500 p-[9px] px-5 font-medium text-white hover:bg-opacity-90">
                            ...
                            {updating ? "Saving..." : "Save Catalogs Page"}
                        </button>
