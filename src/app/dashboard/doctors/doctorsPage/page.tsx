...
const DoctorsPage = () => {
    ...
    const [doctorsPage, setdoctorsPage] = useState<DoctorsPage | null>(null);
    ...
    const [loadingDelete, setLoadingDelete] = useState(false);
      ...
      useEffect(() => {
        ...
        const fetchDoctorsPage = async () => {
          try {
            const response = await fetch(`/api/doctorsPage`, {
              method: "GET",
              headers: {
      ...
      const handleUpdate = async (e: React.FormEvent) => { 
        ...
        formData.append("title", doctorsPage.title);
        formData.append("headline", doctorsPage.headline);
        formData.append("description", doctorsPage.description);
      
        if (image) {
          formData.append("image", image);
        }  

        doctorsPage.keywords.forEach(keywords => formData.append("keywords", keywords));
      
        try {
          const res = await fetch(`/api/doctorsPage`, {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_SECRET_KEY}`,
            },
  ...
  return (
    <DefaultLayout>
      <div className="mb-6 flex flex-col gap-3 sm:flex-col sm:items-start sm:justify-between">
        <Breadcrumb route="dashboard/doctors" pageName="Manage Doctors" routeSecond={`dashboard/doctors/doctorsPage`} pageNameSecond="/ Doctors Page" routeThird="" pageNameThird="" routeFour="" pageNameFour="" routeFive="" pageNameFive="" />
      </div>
                ...
                <form onSubmit={handleUpdate} encType="multipart/form-data">
                    <div className="mb-4 flex flex-col gap-4.5 xl:flex-row ">
                        ...
                        />
                            ...
                            <div className="flex flex-col items-center justify-center">
                                ...
                                {(previewImage || doctorsPage?.image) && (
                                <Image
                                    width={800}
                                    height={800}
                                    src={(previewImage || doctorsPage?.image) as string}
                                    alt="Preview"
                                    priority
                    ...
                    <div className="mb-7 flex flex-col gap-4.5 xl:flex-row">
                        <div className="w-full xl:w-1/2">
                            ...
                            <input
                                type="text"
                                defaultValue={doctorsPage?.title}
                                name="title"
                        ...
                        <div className="w-full xl:w-1/2">
                            ...
                            <input
                                type="text"
                                defaultValue={doctorsPage?.headline}
                                name="headline"
                                onChange={handleChange}
                    ...
                    <div className="mb-6 flex flex-col gap-4.5 xl:flex-row">
                        <div className="w-full xl:w-full">
                            ...
                            <textarea
                                defaultValue={doctorsPage?.description}
                                name="description"
                                onChange={handleChange}
                    ...
                    <div className="flex gap-3">
                        <button type="submit" onClick={handleUpdate} disabled={updating} className="flex w-max gap-2 justify-center rounded-[7px] bg-green-500 p-[9px] px-5 font-medium text-white hover:bg-opacity-90">
                            ...
                            {updating ? "Saving..." : "Save Doctors Page"}
                        </button>
