    ...
    const [previewImage, setPreviewImage] = useState<string | null>(null);
      ...
      useEffect(() => {
        ...
        const fetchArticlesPage = async () => {
          try {
            const response = await fetch(`/api/articlesPage`, {
              method: "GET",
              headers: {
      ...
      const handleUpdate = async (e: React.FormEvent) => { 
        ...
        formData.append("title", articlesPage.title);
        formData.append("headline", articlesPage.headline);
        formData.append("description", articlesPage.description);
      
        if (image) {
          formData.append("image", image);
        }  

        articlesPage.keywords.forEach(keywords => formData.append("keywords", keywords));
      
        try {
          const res = await fetch(`/api/articlesPage`, {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_SECRET_KEY}`,
            },
  ...
  return (
    <DefaultLayout>
      <div className="mb-6 flex flex-col gap-3 sm:flex-col sm:items-start sm:justify-between">
        <Breadcrumb route="dashboard/articles" pageName="Manage Articles" routeSecond={`dashboard/articles/articlesPage`} pageNameSecond="/ Articles Page" routeThird="" pageNameThird="" routeFour="" pageNameFour="" routeFive="" pageNameFive="" />
      </div>
                ...
                <form onSubmit={handleUpdate} encType="multipart/form-data">
                    <div className="mb-4 flex flex-col gap-4.5 xl:flex-row ">
                        ...
                        <div className="flex flex-col items-center justify-center">
                            ...
                            {(previewImage || articlesPage?.image) && (
                            <Image
                                width={800}
                                height={800}
                                src={(previewImage || articlesPage?.image) as string}
                                alt="Preview"
                                priority
                    ...
                    <div className="mb-7 flex flex-col gap-4.5 xl:flex-row">
                        <div className="w-full xl:w-1/2">
                            ...
                            <input
                                type="text"
                                defaultValue={articlesPage?.title}
                                name="title"
                        ...
                        <div className="w-full xl:w-1/2">
                            ...
                            <input
                                type="text"
                                defaultValue={articlesPage?.headline}
                                name="headline"
                                onChange={handleChange}
                    ...
                    <div className="mb-6 flex flex-col gap-4.5 xl:flex-row">
                        <div className="w-full xl:w-full">
                            ...
                            <textarea
                                defaultValue={articlesPage?.description}
                                name="description"
                                onChange={handleChange}
                    ...
                    <div className="flex gap-3">
                        <button type="submit" onClick={handleUpdate} disabled={updating} className="flex w-max gap-2 justify-center rounded-[7px] bg-green-500 p-[9px] px-5 font-medium text-white hover:bg-opacity-90">
                            ...
                            {updating ? "Saving..." : "Save Articles Page"}
                        </button>
