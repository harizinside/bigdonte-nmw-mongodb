...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(`https://nmw.prahwa.net/api/promo/${id}`);
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
    try {
      ...
      const response = await fetch(`https://nmw.prahwa.net/api/promo/update/${id}`, {
        method: "POST", // Sesuai dengan backend utama
        body: formData,
      });
  
      if (!response.ok) {
        throw new Error(`Failed to update promo: ${response.statusText}`);
      }
