...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ message: "Failed to fetch achievement" }, { status: 500 });
  }
}
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(`https://nmw.prahwa.net/api/achievment/update/${id}`, {
      method: "POST", // Sesuai dengan backend utama
      body: formData,
    });

    if (!response.ok) {
      throw new Error(`Failed to update achievement: ${response.statusText}`);
    }
